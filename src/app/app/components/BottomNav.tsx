...
import { Icon, type LegendIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type LegendTab = 'profile' | 'achievements' | 'pro' | 'settings';

export function BottomNav({ active, onSelect }: { active: LegendTab; onSelect: (t: LegendTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: LegendTab; icon: LegendIconName; label: string }[] = [
    { key: 'profile',      icon: 'medal',    label: t.legend.nav.profile      },
    { key: 'achievements', icon: 'trophy',   label: t.legend.nav.achievements },
    { key: 'pro',          icon: 'star',     label: t.legend.nav.pro          },
    { key: 'settings',     icon: 'settings', label: t.legend.nav.settings     },
  ];
  return (
    <nav style={{
